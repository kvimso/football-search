...
const MINI_BATCH_SIZE = 4; // Process 4 clubs per API call to avoid token limits

export async function analyzeBatch(clubs) {
  const allResults = [];

  // Split into mini-batches of 4 clubs
  const batches = [];
  for (let i = 0; i < clubs.length; i += MINI_BATCH_SIZE) {
    batches.push(clubs.slice(i, i + MINI_BATCH_SIZE));
  }

  for (const batch of batches) {
    // Try Claude first (faster per call, better for Vercel 60s timeout)
    try {
      const batchResult = await analyzeClubBatchWithClaude(batch);
      if (batchResult) {
      ...
      console.warn(`Claude mini-batch failed (${batch.map((c) => c.name).join(", ")}):`, err.message);
    }

    // Try Gemini as fallback
    if (isGeminiConfigured()) {
      try {
        const results = await analyzeBatchWithGemini(batch);
        for (const r of results) {
          const club = batch.find((c) => c.name === r.club_name);
          allResults.push({
            club_id: club?.club_id,
        ...
        console.warn(`Gemini mini-batch failed (${batch.map((c) => c.name).join(", ")}):`, err.message);
      }
    }
